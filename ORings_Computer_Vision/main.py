...
def clusteringAlgorithmFunc(img, average):
    newThreshold = average
    oldThreshold = 0
    while abs(newThreshold - oldThreshold) > 0.001:
        ...
        for i in range(0, img.shape[0]):
            for j in range(0, img.shape[1]):
                if img[i, j] > newThreshold:
                    x += img[i, j]
                    amount_of_x += 1
                if img[i, j] <= newThreshold:
                    y += img[i, j]
                    amount_of_y += 1
        newThreshold = (x / amount_of_x + y / amount_of_y) / 2
    return int(newThreshold)


# Applying the threshold value provided by the clustering algorithm
def thresholdFromClusteringAlgorithm(img, thresh):
    copy = img.copy()
        ...
        for j in range(0, copy.shape[1]):
            try:
                if copy[i, j] == 0:
                    ...
                    for k in range(-1, 2):
                        for n in range(-1, 2):
                            if copy[i + k, j + n] == 255 and copy[i + k + 1, j + n] == 255 and copy[
                                i + k - 1, j + n] == 255 and copy[i + k - 1, j + n + 1] == 255 and copy[
                                i + k - 1, j + n - 1] == 255:
                                img[i, j] = 255
            except IndexError:
                break
...
def closing(img):
    eErosion = erosion(img)
    dDilartion = dilation(eErosion)
    return dDilartion
...
for x in range(1, 16):
    ...
    img = cv.imread('./Oring/Oring5.jpg', 0)

    # Function calls
    thresMean = meanOfPixels(img)
    newthresh = clusteringAlgorithmFunc(img, thresMean)
    thresh = thresholdFromClusteringAlgorithm(img, newthresh)
    close = closing(thresh)
    labeling = component_labelling(close)
    center = getCenter(labeling)
    radius = radiusOfORing(labeling, center)
    ...
    if result:
        cv.putText(backtorgb, 'PASS', (10, 217), cv.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv.LINE_AA)
    else:
